...
(function (angular) {
    ...
    //     "total": 47
    // };
    var theatersModule = angular.module('moviecatTevins.movie_list', ['ngRoute', 'moviecatTevins.service.http']);
    theatersModule.config(["$routeProvider", function ($routeProvider) {
        $routeProvider.when('/:category/:pageIndex', {
            controller: 'CategoryController',
            templateUrl: 'movie_list/view.html'
        });
    }]);
    // theatersModule.controller('InTheaterController', ['$scope', '$http', function ($scope, $http) {
    theatersModule.controller('CategoryController', ['$scope', '$route', '$routeParams', 'HttpService', 'AppConfig', function ($scope, $route, $routeParams, HttpService, AppConfig) {
        // console.log("类别", ": " + $routeParams.category);
        $scope.isLoading = true;//是否在加载数据
        $scope.data;//api返回的数据
        $scope.message = '';//错误信息
        var page = parseInt($routeParams.pageIndex);//页数
        var count = AppConfig.pageSize;//单页数据量
        ...
        // });
        var url = AppConfig.listApiAddress + $routeParams.category;//请求地址
        // $routeParams 的数据来源：1.路由匹配出来的，2."?"参数
        var params = {start: startIndex, count: count, q: $routeParams.q};//携带参数
